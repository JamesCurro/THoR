			...
			if not(header.group("event") in badEvents):
				...
				if( ((length == length2 == 6) or (length == length2 == 5) or (length == length2 == 4)) and  (eventLocation in goodLocations)):
					...
					if ((header.group('event') == "SHOT" or header.group('event') == "GOAL")):
						...
						yc_v.append('')
# make rebounds
rebounds = zerosFill(len(eventy))
for x in range(0,len(eventy)):
        if (eventy[x] == 'SHOT' or eventy[x] == 'GOAL') and eventy[x-1] == 'SHOT' and (timey[x-1] - timey[x] < 3):
                rebounds[x] = 1
        else:
                rebounds[x] = 0
...
with open('xAll.csv','w') as xMatrixFile:
	for x in range(0,currentPlay):
		...
		#xMatrixFile.write(',1,'+ str(shiftVector[x])+ ',' +str(scoreEffect[x]) + ',' +str(pp_2_vector[x]) +','+str(shift_pp2[x])+','+str(away_m_home[x])+',\n');
...
with open('timeAllFile.csv','w') as timeFile:
	for x in range(0,len(timeToGoalHome)):
		timeFile.write(str(timeToGoalHome[x]) + "," + str(timeToGoalAway[x]) + "," + str(eventy[x])+ "," + str(timey[x])+ "," + str(periody[x])+ "," + str(gamecodey[x])+ "," + str(goalIndy[x]) + ',' + str(xc_v[x]) + ',' + str(yc_v[x]) + ',' + str(xc_v_adj[x]) + ',' + str(yc_v_adj[x])+ ',' + str(xgrid[x]) + ',' + str(ygrid[x]) + ',' + str(rebounds[x]) + ',\n');
